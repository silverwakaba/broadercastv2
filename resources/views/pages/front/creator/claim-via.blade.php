    ...
    <x-Adminlte.Content :previous="route('creator.claim', ['id' => $profile->identifier])">
        <x-Adminlte.CardForm title="Claim" button="Claim" captcha="0">
            @foreach($tracker->data as $channel)
                <x-Form.Input name="email" type="email" text="Email" placeholder="Enter your email so that we can send you further instructions regarding account access, if the claim process is successful. THIS IS NOT FOR YOUR NEW LOGIN EMAIL ACCESS AS IT'S FOR A SEPARATE PROCESS!" />
