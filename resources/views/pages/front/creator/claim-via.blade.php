...
@section('content')
    <x-Adminlte.Content :previous="route('creator.claim', ['id' => $profile->identifier])">
        <x-Adminlte.CardForm title="Claim" button="Submit">
            @foreach($tracker->data as $channel)
                <x-Form.Input name="email" type="email" text="Email" placeholder="Enter your email for further instruction regarding account access" />
